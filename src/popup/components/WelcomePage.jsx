import { getQueryStringFromUrlAsObject, slugify } from "@/global/fn"
...
import { courseUrlFromSlug } from "@/global/class/course-api/course_fn"
...
class WelcomePage extends ComponentWithMessaging {
  ...
  async insertCourseLegacy(course, sections) {
    if (course) {
      // create Course
      const { title, slug, duration, sourceCodeRepository, description, urn } =
        course
  ...
  async addCourseFromCurrentUrl() {
    ...
    const courseSlug = url
      .replace("https://www.linkedin.com/learning/", "")
      .split("/")[0]
      ...
      <div className="relative">
        ...
        <div className="welcome-page page gap-2 flex flex-col items-center pt-2">
          ...
          <div className="action-cnt w-full">
            ...
            {validCoursePage ? (
              <div className="btn-cnt text-center flex gap-2 p-2">
                <CheckBox
                  label="Legacy Mode"
                  className="pt-2"
                  checked={legacyMode}
                ...
                <Button
                  disabled={
                    fetchBtnState == 1 || !validCoursePage || disableFetchBtn
                  }
